...
const allNavItems = [
    ...
    {
        name: 'Agent Metrics',
        route: 'agents.metrics.index',
        description: 'Live CPU, memory, disk, and services from enrolled Windows agents',
...
const routeMeta = computed(() => {
    ...
    if (current.startsWith('agents.metrics.')) {
        return {
            section: 'Agent Metrics',
            detail: 'Live CPU, memory, disk, and services from enrolled Windows agents',
        };
    }
...
function isActive(itemRoute) {
    ...
    if (itemRoute === 'agents.metrics.index') {
        return current.startsWith('agents.metrics.');
    }
